...
void produce_qyf_onion_key(const char *srcId_string, const char *dstId_string, int index, int time_hour, char *outputqyf) {
    ...
    log_notice(LD_GENERAL, "----- produce_qyf_onion_key4 seedstr:  %d",time_hour);
    snprintf(seed_str, sizeof(seed_str), "%s%s%d%d", srcId, dstId, index, time_hour);
...
void kill_uploaded_onion(const unsigned char *onion) {
  ...
  if ((onion[0] != '\0') && hs_address_is_valid(onion) && (uploaded_qyf == 1)) {
      log_notice(LD_GENERAL,"----- kill_uploaded_onion is SUCCESSS onion is %s ------------", onion);
