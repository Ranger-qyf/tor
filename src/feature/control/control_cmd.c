...
{
  log_notice(LD_GENERAL, "-----%s is starting....",__FUNCTION__);
                               ...
                               const control_cmd_args_t *args)
{
  ...
  log_notice(LD_GENERAL,"-----%s  parsedescriptorthree_helper is over!!!",__FUNCTION__);
...
control_transmithiddenservicedescriptor_helper(const control_cmd_args_t *args)
{
  ...
  if(number_of_services >= 128){
  	log_notice(LD_GENERAL,"-----%s A TOR process can create up to 128 hidden services",__FUNCTION__);
	return;
  }
  if (descriptor_len_zrm > 0){
    ...
    memcpy(hidden_service_desc_temp_zrm,smartlist_get(args->args,0), descriptor_len_zrm);
	 //----------zrm--------start--------
    log_notice(LD_GENERAL,"-----%s the zrm section is starting",__FUNCTION__);
    ...
    log_notice(LD_GENERAL,"-----%s hidden_service_descriptor[%d][0] %s  ------------",__FUNCTION__,number_of_services, desc_padding);
    log_notice(LD_GENERAL,"-----%s desc_padding is %s  ------------",__FUNCTION__, desc_padding);
    log_notice(LD_GENERAL,"-----%s desc_padding , len is  %s  ------------",__FUNCTION__, hidden_service_descriptor_v3_zrm_list[number_of_services][0]);
    int count = 1;
    while (desc_padding != NULL) {
      ...
      log_notice(LD_GENERAL,"-----%s hidden_service_descriptor[%d][%d] %s  ------------",__FUNCTION__,number_of_services,count, desc_padding);
      //log_notice(LD_GENERAL,"-----%s desc_padding , assignment success ------------",__FUNCTION__);
      ++count;
      log_notice(LD_GENERAL,"-----%s  loop number of count is: %d",__FUNCTION__, count);
      if(count >= 128){
          log_notice(LD_GENERAL,"-----%s the embeding content's length is too long, we will upload the first 2.5 MB of the content  ------------",__FUNCTION__);
	        break;
      }
    }
    log_notice(LD_GENERAL,"-----%s  number of count is: %d",__FUNCTION__, count);
                               ...
                               const control_cmd_args_t *args)
{
  ...
  log_notice(LD_GENERAL,"%s   onion_address_zqf is %s\tonion_id_zqf is %d",__FUNCTION__,onion_adddress_zqf,onion_id_zqf);
  send_control_done(conn);
  return 0;
}
// --------------------------------hwt ----------------------------
...
handle_control_updatenewconsensus(control_connection_t *conn,
                                     const control_cmd_args_t *args)
{
  log_notice(LD_GENERAL,"%s  hwt update new consensus starting...",__FUNCTION__);
  update_networkstatus_downloads_hwt(time(NULL));
  log_notice(LD_GENERAL,"%s  hwt update new consensus end",__FUNCTION__);
  send_control_done(conn);
  return 0;
}
                                     ...
                                     const control_cmd_args_t *args)
{
  ...
  log_notice(LD_GENERAL,"%s   fingerprint_zqf is %s",__FUNCTION__,fingerprint_zqf);
  send_control_done(conn);
  return 0;
}
                         ...
                         const control_cmd_args_t *args)
{
  ...
  log_notice(LD_GENERAL, "-----%s %s 生成onion_sk完成:%s....", __FUNCTION__,onionkey,pk.v3->seckey);
  if (ed25519_public_key_generate(onion_pk,
                                  pk.v3) < 0) {
    log_warn(LD_CONFIG, "Unable to generate ed25519 public key"
                        "for v3 service.");
    log_notice(LD_GENERAL, "-----%s Unable to generate ed25519 public key", __FUNCTION__);
    goto out;
  }//hwt_定位onion公钥生成
                         ...
                         const control_cmd_args_t *args)
{
  ...
  /************yfq */
    if (!strcasecmp(arg->key, "SumOfReplica")) {
      ...
      log_notice(LD_GENERAL, "-----%s sum_of_replica is %d", __FUNCTION__, sum_of_replica);
    }
  ...
  /************yfq */
      ...
      log_notice(LD_GENERAL, "-----%s number_of_onions is %d", __FUNCTION__, number_of_onions);
      /************yfq */
      if (!cfg) {
