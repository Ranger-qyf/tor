...
static void cookie_by_zqf(origin_circuit_t *circ,char* to) 
{
  ...
  log_notice(LD_GENERAL,"%s    ---------onion_address_zqf is %s\tsizeof(onion_address_zqf) is %d",__FUNCTION__,onion_address_zqf,strlen(onion_address_zqf));  // --------------------------
  unsigned short int id_zqf = get_id_by_onion_zqf(mymap_zqf,onion_address_zqf);
  memcpy(to,&buf_zqf,3);
  memcpy(to+3,&id_zqf,2);
  log_notice(LD_GENERAL,"%s    ---------to id is: %d",__FUNCTION__,*(unsigned short*)(to+3));
