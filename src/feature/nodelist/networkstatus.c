...
{//hwt_定位consensus_down new consensus
  int i;
  const or_options_t *options = get_options();
  //hwt-----time_to_download_next_consensus[i]代表着应该更新consensus的时间，
  //将该值设为now代表现在就该更新共识了，后续tor会一直更新共识，直到网络不通或者下载了更新的共识文件
  log_notice(LD_GENERAL, "%s 将time_to_download_next_consensus[i]设为now...",__FUNCTION__);
  for (i=0; i < N_CONSENSUS_FLAVORS; ++i) {
    time_to_download_next_consensus[i]=now;
  }
  log_notice(LD_GENERAL, "%s 将time_to_download_next_consensus[i]设为now完成",__FUNCTION__);
  if (should_delay_dir_fetches(options, NULL))
